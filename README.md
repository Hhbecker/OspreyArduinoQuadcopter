...
#### Integral term
The integral term is the integration of the error over time. With every execution of the loop, the current error is added to the sum of all past errors. The integral term is necessary to overcome what is known as steady state error. Steady state error occurs when an equilibrium is reached with the proportional term and an opposing force at a state of nonzero error. This can happen because when there is a small error term, the proprotional term is also small. Therefore, the contribution of the proportional term alone may not be enough to correct this small error. The integral term accumulates this residual error over time to drive the system toward its commanded value. Integral terms are tuned to be inconsequential in the transient response  and only become significant over time. Integral windup occurs when the integral term saturates the motor's capabilities and must be considered when designing a PID controller. A simple strategy to avoid integral windup is to cap the integral term at a predetermined value.
